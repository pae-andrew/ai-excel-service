...
Кнопка "В Google Sheets" на результате гонит файл в новую таблицу через OAuth.
Нужен свой проект в Google Cloud Console: OAuth consent screen + OAuth client ID
(Web application), redirect URI = `GOOGLE_REDIRECT_URI` из `.env`, плюс включить
Google Sheets API. Без `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` кнопка отдаёт
понятную ошибку, остальной сервис не страдает.
